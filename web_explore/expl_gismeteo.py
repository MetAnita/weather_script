...
sample_dict = {'Alūksne': 'aluksne-4124'}
val = sample_dict['Alūksne']
...
# vidējā diennakts temperatūra   class="widget-row-chart widget-row-chart-temperature-avg"
...
# Nokrišņu daudzums, mm
print('test the rain ')
test_rain = soup.find_all('div', ["row-item"])
for rain in test_rain:
    value = rain.text
    print(value.strip())

print('new website test the rain')

new_rain = soup.find_all('div', ["widget-row widget-row-precipitation-bars row-with-caption"])
...
print('new website test the rain - 10 day forecast')  # modified soup elemets to fit new website

rain_tag = soup.find_all('div', ["widget-row widget-row-precipitation-bars row-with-caption"])
print(rain_tag)
rain_val = rain_tag[0].find_all('div', ["row-item"])
for rain in rain_val[1:-3]:
    value = rain.text
    print(value.strip())